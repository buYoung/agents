...
- bash는 반환 경로 존재 확인 같은 읽기 전용 사실 확인만 한다. 쓰기, 설치, 빌드, 테스트, 네트워크 명령은 실행하지 않는다.
...
artifact-writing leaf에는 위 네 항목에 더해 taskId, workItemId, 정확히 하나의 독립된 `Output: .agents/<taskId>/<workItemId>/<role-file>.md` 줄, 필요한 0개 이상의 독립된 `Input: ...` 줄을 넣는다. Output만 active writable assignment이며 Input은 읽기 전용이다.
...
- taskId나 role을 바꾸려면 새 leaf thread를 만든다. 모든 `worker` message에는 요청 산출물과 할당된 `.agents/<taskId>/<workItemId>/work.md` 외의 임의 파일을 수정하지 말라는 강제 제약을 넣는다.
...
- 존재 확인이 꼭 필요하면 반환된 `.agents/<taskId>/<workItemId>/` 또는 docs 경로에 `test -f`나 `wc -l` 같은 읽기 전용 확인만 한다. artifact directory scan이나 본문 읽기는 하지 않는다.
- main session은 첫 leaf 반환 뒤에만 예약한 exact `.agents/<taskId>/<coordinatorWorkItemId>/task.md`를 생성하거나 갱신해 delegation path와 summary index를 기록한다. 현재 runtime이 쓰기를 허용하지 않으면 파일 소유를 주장하지 말고 paths-only 결과로 끝낸다.
...
```text
Path: .agents/<taskId>/<workItemId>/<role-file>.md
Summary: <한 줄 요약>
```
...
- 모든 run file은 canonical `.agents/<taskId>/<workItemId>/` 범위 안에만 둔다.
