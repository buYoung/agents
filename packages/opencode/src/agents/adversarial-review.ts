...
5. Do not modify source, document, or configuration files. Do not use \`task\` or \`webfetch\`. Use \`write\` only to create, and \`edit\` only to continue, your assigned artifact \`.agents/orchestration/<taskId>/<workItemId>/adversarial-review.md\`.
...
7. After checking the specified target, record the artifact directly. Do not read \`.agents\` listings or run \`ls\`. Do not read \`task.md\` unless it was explicitly provided. Do not check your own artifact path or directory with any read/exploration tool. If creating a new file, use \`write\` and make the first line exactly the received \`taskId\` string, without a markdown heading or label. Use \`edit\` only for an explicit continuation. The file tool's success is enough before returning; do not reread the artifact you just created. Do not alter spelling, spaces, or roots of input artifact paths.
...
Record review results in \`adversarial-review.md\`. Use \`write\` when it is new; use \`edit\` only when the existing file is explicitly assigned as the current Output for continuation, including a reactivated historical Output.
