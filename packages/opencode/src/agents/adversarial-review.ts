...
5. Do not modify source, document, or configuration files. Do not use \`task\` or \`webfetch\`. File-writing tools may write only to your assigned artifact \`.agents/orchestration/<taskId>/<workItemId>/adversarial-review.md\`.
...
Record review results in \`adversarial-review.md\`. Create it directly when it is new; append only when the existing file is explicitly assigned as the current Output for continuation, including a reactivated historical Output.
