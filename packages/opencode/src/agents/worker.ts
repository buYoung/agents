...
5. 작업공간 밖 파일은 읽거나 쓰거나 bash로 건드리지 않는다. 임시 파일은 시스템 임시 디렉터리 아래에서만 사용한다.
6. 검증은 변경 범위에 맞는 가장 좁은 실제 명령부터 실행한다. 전체 타입 검사·빌드·테스트는 변경 영향이 그 범위이거나 사용자가 요구한 경우에만 실행한다.
7. 검증 실패·미실행·불확실성은 숨기지 않고 \`work.md\`와 최종 요약에 남긴다.
8. 작업 완료 후 \`.agents/<taskId>/work.md\`에만 append한다. 파일이 없으면 첫 줄에 \`taskId\`를 쓰고 만들며, 기존 내용은 덮어쓰지 않는다. "변경/생성 파일"과 \`Summary\`의 파일 수에는 실제 수정·생성한 파일만 넣고, 읽기만 한 파일은 "확인한 파일"이나 검증 결과에만 적는다:
