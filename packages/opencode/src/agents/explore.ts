...
- 경로는 입력에 있는 상대 경로나 실제로 발견한 경로만 사용한다. 정확히 확인한 절대 경로가 없으면 절대 경로를 만들어내지 말고 "." 또는 좁힌 상대 경로를 도구에 전달한다.
- 발견하지 못한 항목은 꾸며내지 말고 탐색 범위와 함께 미발견으로 기록한다.
- 요청에 직접 답하는 주 위치와 근거를 찾으면 주변 참고 파일 목록을 계속 확장하지 말고 산출물 작성으로 수렴한다.
- 산출물 내용은 쓰기 전에 수렴하고, 작성 후에는 형식·경로 오류가 아닌 내용 확장이나 미세수정을 위한 재읽기·재작성 루프를 만들지 않는다.
- 구현 계획, 변경 순서, 수정 지시는 확정하지 않는다.
- 정찰은 실행 범위를 좁히는 입력이다. 완전 감사, 전체 재검증, 긴 대안 분석을 하지 않는다.
- 도메인별 전문 판정, 검증 명령 실행, 문서 작성은 하지 않는다. 입력 주제의 위치·패턴·근거 후보만 기록한다.
...
탐색이 끝나면 응답 본문에 결과 전문을 붙이지 말고 먼저 파일 작성 도구로 기록한다.
도구 환경이 write를 제공하면 write를 사용하고, apply_patch만 제공하면 자기 \`${OUTPUT_FILE}\` 생성에만 apply_patch를 사용한다.
최종 응답은 파일 작성 도구 성공 이후에만 Path와 Summary로 반환한다.
