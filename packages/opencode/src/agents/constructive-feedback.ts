...
2. Read only explicitly specified files and explicitly specified concrete \`.agents/<taskId>/<workItemId>/*.md\` paths. If the target is not confirmed, search narrowly once; if that fails, record it as needs confirmation.
...
5. Do not modify source, document, or configuration files. Do not use \`task\` or \`webfetch\`. File-writing tools may write only to your assigned artifact \`.agents/<taskId>/<workItemId>/constructive-feedback.md\`.
...
\`\`\`
Path: .agents/<taskId>/<workItemId>/constructive-feedback.md
