...
| Historical Output | 같은 planner 세션의 이전 활성 출력으로서, 같은 taskId·역할의 명시적 재할당으로 다시 활성화되기 전에는 읽기만 가능한 산출물 |
...
`planner`는 `subagent` 실행 모드다. 오케스트레이터에게 받은 taskId, workItemId, 정확히 하나의 활성 `Output:` 경로가 모두 유효할 때만 계획을 진행한다. 0개 이상의 `Input:`과 같은 세션의 이전 Output 이력은 읽을 수 있지만 쓸 수 없다. 같은 세션에서 새 work item을 받으면 taskId와 역할이 같은 경우에만 새 Output이 활성화되고 이전 Output은 읽기 전용 이력이 된다. 같은 taskId·역할의 명시적 후속 실행이 이전 이력 Output을 현재 Output으로 다시 할당하면 해당 Output은 활성·쓰기 가능 상태로 돌아가고 직전 활성 Output은 이력이 된다. 소스 읽기와 제한적 명령 실행은 가능하지만 소스 변경, 웹 조회, 재위임은 허용되지 않는다. 산출물은 실행 계획과 영향 범위를 담는 `plan.md`다.
...
- Input이나 현재 Output으로 명시적으로 재활성화되지 않은 이력에 쓰려는 시도는 거부된다.
...
- 이전 이력이 현재 Output으로 명시적으로 재할당되면 그 Output은 쓰기 가능해지고 직전 활성 Output은 읽기 전용 이력이 된다.
...
- 직접 파일 읽기와 read-only bash 모두 같은 세션의 활성 Output, 이력 Output, 명시적 Input으로 읽기 범위를 제한한다.
...
- 같은 taskId·역할의 명시적 historical Output 재활성화.
...
- 같은 taskId·역할의 명시적 후속 실행이 historical Output을 현재 Output으로 재할당하면 해당 Output은 다시 쓰기 가능해지고 직전 활성 Output은 이력이 된다.

### 14.2 OpenCode

- 부모 task 연속 실행과 lifecycle metadata가 활성 work item 전이를 runtime에서 승인한다.
- Input과 이력 Output의 읽기 및 활성 Output 쓰기를 실행 전 권한 경계가 확인하며, read-only bash에도 같은 readable set을 적용한다.

### 14.3 Codex

- planner leaf의 명시적 후속 메시지는 같은 taskId·역할의 historical Output 재활성화 의미를 따르지만 OpenCode 전용 task 필드를 사용하지 않는다.
...
| 2026-07-10 | updated | 직접 읽기와 bash의 readable set을 일치시키고 OpenCode와 Codex의 historical Output 명시적 재활성화 의미를 구체화했다. |
