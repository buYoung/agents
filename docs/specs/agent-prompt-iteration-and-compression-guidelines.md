...
4. 각 핵심 fixture 유형은 먼저 warmup 1회를 실행해 하네스, 모델, 권한, 산출물 경로, fallback, MCP 설정이 정상인지 확인한다.
...
10. 같은 fixture 유형 요청을 clean-run 3회로 검증한다. 가능하면 3개 run을 병렬 실행해도 된다.
11. 3회 모두 같은 계약을 지키면 그 유형을 통과로 본다.
12. 실패가 재발하면 다시 6단계로 돌아간다.

1회 성공은 안정성 근거가 아니다. 모델 변동성이 있으므로 최소 3회 반복 평균과 실패 양상을 함께 기록한다.

warmup 1회는 탐침일 뿐 완료 근거가 아니다. warmup 통과를 3회 반복 통과 중 1회로 계산하지 않는다.

clean-run 3회 병렬 실행 규칙:

- 병렬 실행은 같은 fixture 유형의 최종 판정용 3회 반복에만 사용한다.
...
- 3회 중 1회라도 실패하면 해당 fixture 유형 전체를 실패로 보고, 최소 보강과 압축 후 새 run id로 3회를 다시 시작한다.
- 여러 fixture 유형을 한꺼번에 9개 이상 병렬 실행하는 것은 비용, 속도 제한, 로그 관리 위험이 크므로 기본 흐름으로 삼지 않는다.
...
- 같은 fixture 유형의 clean-run 3회는 서로 독립된 run id와 taskId를 사용한다면 병렬 실행할 수 있다.
