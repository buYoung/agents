...
| `pending` | 아직 시작하지 않음 |
| `in-progress` | 현재 작업 중 |
| `needs-clean-revalidation` | 실패 분석 컨텍스트가 평가에 섞였을 가능성이 있어 새 세션 3회 재검증 필요 |
...
- 실패 로그와 도구 출력이 다음 평가 입력에 누적되어 환각 재료가 되는 문제
...
- [ ] 각 핵심 fixture 유형은 최종 3회 반복 전에 warmup 1회를 실행한다.
- [ ] warmup에서는 하네스, 모델, 권한, 산출물 경로, fallback, MCP 설정을 가볍게 확인한다.
- [ ] warmup 결과는 완료 근거나 3회 반복 통과 횟수에 포함하지 않는다.
...
- [ ] 최종 판정용 3회 반복은 가능하면 같은 fixture 유형 단위로 병렬 실행한다.
- [ ] 병렬 run은 각각 다른 `OPENCODE_RUN_ID`, opencode 데이터베이스, `taskId`, 산출물 경로를 사용한다.
...
- [ ] 병렬 3회 중 1회라도 실패하면 해당 fixture 유형 전체를 실패로 보고, 새 run id로 3회 반복을 다시 시작한다.
...
- 보강 내용: OpenAI 계열 도구 환경에서 `.agents/<taskId>/ideas.md` 산출물 생성이 `apply_patch`로 들어오는 경우를 자기 산출물에 한정해 허용하고, `apply_patch` 경로 추출이 `patchText`와 `*** Add File:` 단일 경로를 인식하게 했다.
- 추가 보강: `MCP 없음` 기준선에서 불필요한 `todowrite`가 2/3회 관찰되어 todo, 진행 목록, 상태 관리 도구 금지를 추가했다.
- 최종 도구 판정: `codemap-search` 사용 조건 6/6 충족, `MCP 없음` 대체 읽기 도구 3/3, `bash`/`webfetch`/`edit`/`task`/`todowrite` 0/9.
- 프롬프트 압축 완료: 기준 `IDEATOR_PROMPT` 4,055자, 보강 후 4,691자, 압축 후 최종 4,019자.
- 압축 후 clean-run 재검증 완료: `idea-generator-compressed2-gpt54mini-mcp-normal-1..3` 3/3, `idea-generator-compressed4-gpt54mini-mcp-boundary-1..3` 3/3, `idea-generator-compressed2-gpt54mini-nomcp-normal-1..3` 3/3.
...
- [x] 결함, 반례, 회귀, 보안, 호환성 위험을 우선 찾는다.
- [x] 직접 수정하지 않는다.
- [x] 최종 승인/불승인 결정을 대신하지 않는다.
- [x] 근거와 심각도를 분리해 기록한다.
- [x] 발견 없음과 검토 불충분을 구분한다.
...
- 오케스트레이터 위임 캡처 완료: `adversarial-delegation2-gpt53-spark-20260708`에서 실제 위임 실패 형태를 확인했다. `subagent_type` 누락과 구체 대상 경로 누락이 관찰되어, 직접 평가 입력은 `taskId`, 명시 대상 파일, 자기 산출물 경로, `Path`/`Summary` 반환으로 고정했다.
- 프롬프트 압축 완료: 기준 `ADVERSARIAL_REVIEW_PROMPT` 1,435자, 보강 최고 1,671자, 압축 후 최종 1,330자.
...
- 보강 내용: 명시 대상만 검토, 인접 파일·git 이력·모델 설정 과탐색 금지, `.agents` 목록 탐색 금지, 비활성 MCP 명령의 `bash` 우회 차단, 산출물 경로 보존, 확인 근거와 추론 분리.
...
- [ ] 핵심 fixture 3회 반복 통과
- [ ] 병렬 실행을 사용했다면 run id, taskId, output log 분리 기록
- [ ] 실패 후 재평가라면 실패 분석 컨텍스트와 분리된 clean-run 3회 반복 통과
