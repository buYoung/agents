...
따라서 `intent-checker`, `research`, `explore`, `ideator`, `planner`, `adversarial-review`, `constructive-feedback`의 직접 계약 평가는 `scripts/run-opencode --direct-subagent <agent> run ...` 평가 전용 하네스로 수행한다. 이 하네스는 운영 mode를 바꾸지 않고, 평가 실행에서만 target agent를 직접 선택 가능하게 해야 하며, 권한 정책은 대상 agent의 원래 정책을 유지해야 한다.
