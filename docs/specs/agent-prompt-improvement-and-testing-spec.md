...
| Fixture | Role fidelity | Boundary control | Output contract | Uncertainty handling | Notes |
| ------- | ------------- | ---------------- | --------------- | -------------------- | ----- |
...
5. 정적 계약 테스트를 추가하거나 갱신한다.
6. 타입 및 패키지 검증을 실행한다.
7. 행동 평가 fixture를 실행하거나 수동 평가한다.
8. 실패한 fixture가 있으면 prompt를 줄이고 명확하게 수정한다.
