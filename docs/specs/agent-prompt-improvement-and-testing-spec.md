...
| `idea-generator` | `docs/FDD/agent-ideator-role.md` |
...
따라서 `intent-checker`, `research`, `code-explorer`, `idea-generator`, `planner`, `adversarial-review`, `constructive-feedback`의 직접 계약 평가는 `scripts/run-opencode --direct-subagent <agent> run ...` 평가 전용 하네스로 수행한다. 이 하네스는 운영 mode를 바꾸지 않고, 평가 실행에서만 target agent를 직접 선택 가능하게 해야 하며, 권한 정책은 대상 agent의 원래 정책을 유지해야 한다.
...
5. 실행 가능한 평가 모드를 확정한다. `subagent` 직접 평가는 `scripts/run-opencode --direct-subagent <agent>` 같은 평가 전용 하네스 없이는 수행하지 않는다.
